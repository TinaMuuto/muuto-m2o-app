...
PRICE_MATRIX_UK_XLSX_PATH = os.path.join(BASE_DIR, "price-matrix_UK-EI.xlsx") # New Path for UK/EI prices
...
def load_data():
    ...
    if os.path.exists(RAW_DATA_XLSX_PATH):
        try:
            ...
            data_load_errors.append(f"Error loading Raw Data: {e}")
    else:
    ...
    price_matrices_to_load = {
        ...
        'uk_ei': PRICE_MATRIX_UK_XLSX_PATH
    }
    ws_dfs = {}
    rt_dfs = {}

    for market, path in price_matrices_to_load.items():
        if os.path.exists(path):
            try:
                ws_dfs[market] = pd.read_excel(path, sheet_name=PRICE_MATRIX_WHOLESALE_SHEET)
                rt_dfs[market] = pd.read_excel(path, sheet_name=PRICE_MATRIX_RETAIL_SHEET)
            except Exception as e:
                data_load_errors.append(f"Error loading price data from {os.path.basename(path)}: {e}")
        else:
            # Only error if the EUROPE one is missing, as UK is an addition
            if market == 'europe':
                data_load_errors.append(f"Price Matrix file not found: {os.path.basename(path)}")
    
    # Merge price dataframes if they exist
    if ws_dfs:
        base_ws_df = ws_dfs.get('europe')
        if base_ws_df is not None:
            article_no_col = base_ws_df.columns[0]
            if 'uk_ei' in ws_dfs:
                ...
                uk_cols_to_use = [article_no_col] + [col for col in uk_ws_df.columns if col not in base_ws_df.columns]
                base_ws_df = pd.merge(base_ws_df, uk_ws_df[uk_cols_to_use], on=article_no_col, how='outer')
        wholesale_prices_df = base_ws_df

    if rt_dfs:
        base_rt_df = rt_dfs.get('europe')
        if base_rt_df is not None:
            ...
            if 'uk_ei' in rt_dfs:
                ...
                # Exclude common columns from the second df before merge, except for the key
                uk_cols_to_use = [article_no_col] + [col for col in uk_rt_df.columns if col not in base_rt_df.columns]
                base_rt_df = pd.merge(base_rt_df, uk_rt_df[uk_cols_to_use], on=article_no_col, how='outer')
    ...
    if os.path.exists(MASTERDATA_TEMPLATE_XLSX_PATH):
        try:
            ...
            data_load_errors.append(f"Error loading Template: {e}")
    else:
        data_load_errors.append(f"Template file not found: {MASTERDATA_TEMPLATE_XLSX_PATH}")

    return raw_df_original, wholesale_prices_df, retail_prices_df, template_cols, data_load_errors

# Load data and check for errors
raw_df_original, wholesale_prices_df, retail_prices_df, template_cols, data_load_errors = load_data()

if data_load_errors:
    for error in data_load_errors:
        st.error(error)
    files_loaded_successfully = False
else:
    st.session_state.raw_df_original = raw_df_original
    st.session_state.wholesale_prices_df = wholesale_prices_df
    st.session_state.retail_prices_df = retail_prices_df
...
if files_loaded_successfully:
    ...
    try:
        if st.session_state.wholesale_prices_df is not None and not st.session_state.wholesale_prices_df.empty:
            ...
            currency_options = [DEFAULT_NO_SELECTION] + sorted([col for col in st.session_state.wholesale_prices_df.columns if str(col).lower() != str(article_no_col_name_ws).lower()])
        else:
            currency_options = [DEFAULT_NO_SELECTION]
            st.error("Wholesale price matrix is empty or could not be loaded.")
    ...
    # Filter raw_df based on currency selection
    if st.session_state.selected_currency_session:
        if st.session_state.selected_currency_session.upper() in ['GBP', 'EI', 'IE']:
        ...
        st.session_state.raw_df = None # Don't show products until currency is selected

    # Check if a currency is selected before proceeding
    if st.session_state.raw_df is not None:

        st.markdown("---")
        # --- Step 2 (Old Step 1): Select product combinations ---
        ...
        if selected_family and selected_family != DEFAULT_NO_SELECTION and 'Product Family' in df_for_display.columns:
            ...
            if not family_df.empty and 'Upholstery Type' in family_df.columns:
                ...
                if not products_in_family: st.info(f"No products in the family: {selected_family}")
                    ...
                    # Matrix Display Logic (as before)
        ...
        def prepare_excel_for_download_final():
            ...
            if not current_selected_currency_for_dl: return None
            ...
            for combo_selection in st.session_state.final_items_for_download:
                ...
                # Use the original (unfiltered) dataframe for final data lookup
        ...
        if can_download_now:
            ...
            if file_bytes:
                st.download_button(
                    ...
                    key="final_download_action_button_v8",
                    help="Click to generate and download your customized master data file."
                )
        else:
            st.button("Generate", key="generate_file_disabled_button_v6", disabled=True, help="Please ensure a currency is chosen (Step 1) and items are selected (Step 2 & 3).")

    else:
        st.info("Please select a currency in Step 1 to see available products.")

else:
    st.error("One or more essential data files could not be loaded correctly. The application cannot continue. Please check the file paths, formats, and column names in your .xlsx files.")

# --- Styling (as before, no changes needed here) ---
st.markdown("""
<style>
